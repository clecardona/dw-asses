...
export default function FieldAdditional({
  ...
  const [quantity, setQuantity] = useState(
    Array(form.additional_info[target].length).fill(0)
  );
  ...
  console.log(form.additional_info);
  ...
  const Inputs = quantity.map((item, index) => (
    <li className="vertical-input" key={index}>
      <input
        type="text"
        defaultValue={
          form.additional_info ? form.additional_info[target][index] : ""
        }
        {...register(`${index}${target}`, { minLength: 3 })}
      />
      {errors[`${index}${target}`] && (
        <p className="input-error">Enter valid field ({">"} 3 chars) </p>
      )}
      {quantity.length > 1 && (
        <ButtonRemove onClick={(index) => removeElement(index)} />
      )}
    </li>
  ));
